...
parser.add_argument('--logNormalize',type=str2bool,default=True,const=True,nargs='?',help='boolean value indicating wheather to log normalize data or not before running bayespace on visium data')
parser.add_argument('--enhancedRes',type=str2bool,default=False,const=True, nargs='?',help='boolean valud indicating to use enhance clusterin or not for running bayespace on visium data')
     ...
     #       args.outdir=os.getcwd()
      #  if(os.path.isdir(args.outdir)==False):
       #     raise ValueError("Directory --outdir does not exist")
        ...
        cmd6 = "Rscript {args.clus} {args.outdir} {args.simpleGridsPath} {args.slidingGridsPath} {args.geneCount1} {args.geneCount2} {args.nFeaturePlotOnly}".format(args=args)
        ret = os.system(cmd6)
        if(ret!=0):
