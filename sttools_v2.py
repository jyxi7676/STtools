####need to parser the path of each script.
...
def step2():
    ...
    cmd2 = "bash align/align_v2.sh -a {args.fq1} -b {args.fq2} -w {args.whitelist} -o {args.outprefix} -t {args.star_path} -q {args.seqtk_path} -g {args.genome}".format(args=args) 
    ret = os.system(cmd2)
    if ( ret != 0 ):
...
def step3():
    ...
    print('hdmi2ndSeq' in vars(args))
    if(('hdmi2ndSeq' in vars(args)) ==False):
        print('add hdmi2ndseq')
        args.hdmi2ndSeq=os.getcwd()+"/HDMI_SeqScope_2nd.txt"
        print(args.hdmi2ndSeq)

    print('hdmi2ndSeq' in vars(args))
    if(('spatial' in vars(args)) ==False):
    ...
    cmd3="{args.py} estimateTissueBoundary/estimateTissueBoundary.py {args.spatial} {args.hdmi2ndSeq} {args.maxScale} {args.outpath} ".format(args=args)
    ret = os.system(cmd3)
    if ( ret != 0 ):
...
def step4():
    ...
    if(os.path.isfile(args.collapsePath)==False):
        raise ValueError("Directory --collapsePath does not exist")
    if(('spatial' in vars(args))==False):
        spatial=os.getcwd()+"/spatialcoordinates.txt"
    cmd4="Rscript getSimpleGrid/simpleGrid_v2.R {args.seqscope1st} {args.DGEdir} {args.spatial} {args.tiles} {args.nrow} {args.ncol} {args.sidesize} {args.outpath} {args.collapseP\
